...
def feed_forward_ReLU(X, params):
    ...
    cache["A3"] = ReLU(cache["Z3"])  # np.exp(cache["Z3"]) / np.sum(np.exp(cache["Z3"]), axis=0)
    return cache
...
def back_propagate_ReLU(X, Y, params, cache, m):
    dZ3 = np.multiply(cache["A3"] - Y, step(cache["Z3"]))
    dW3 = (1./m) * np.matmul(dZ3, cache["A2"].T)
    db3 = (1./m) * np.sum(dZ3, axis=1, keepdims=True)

    dA2 = np.matmul(params["W3"].T, dZ3)
    dZ2 = np.multiply(dA2, step(cache["Z2"]))
    dW2 = (1./m) * np.matmul(dZ2, cache["A1"].T)
    db2 = (1./m) * np.sum(dZ2, axis=1, keepdims=True)

    dA1 = np.matmul(params["W2"].T, dZ2)
    dZ1 = np.multiply(dA1, step(cache["Z1"]))
